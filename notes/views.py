...
def download_note(request, note_id):
    try:
        ...
        file_handle = note.pdf_file.open() # 'pdf_file' is your field name
        
        # Guess the content type (e.g., application/pdf or application/vnd.openxml...)
        content_type, encoding = mimetypes.guess_type(note.pdf_file.name)
...
def get_notes_by_teacher(request, teacher_id):
    ...
    # Inside your data formatting loops:
    data = [
        {
            'id': note.id,
            'filename': os.path.basename(note.pdf_file.name),
            'extension': os.path.splitext(note.pdf_file.name)[1].lower(), # Returns '.docx' or '.pdf'
...
def ask_debate(request):
    try:
        ...
        judge_model = data.get("judge_model", 'nvidia/nemotron-nano-12b-v2-vl:free')
